...
export const input = {
  ...
  spellCast: null,
  grenadeCancel: false,
  grenadeInCancel: false,     // touch: the aiming finger is over the ✕ zone
...
export const controls = {
  ...
  // Drag an aimed grenade onto this and let go to cancel the throw.
  gcancel: { x: 0, y: 0, r: 42, label: '', pressed: false },
...
export function initInput(canvas) {
  ...
  canvas.addEventListener('pointerdown', (ev) => {
    ...
    // Right-click while holding a grenade cancels the throw.
    if (ev.button === 2 && input.grenade) { input.grenadeCancel = true; ev.preventDefault(); return; }
  ...
  canvas.addEventListener('pointermove', (ev) => {
    ...
    if (role === 'stick') {
      ...
      input.grenadeInCancel = dist(x, y, controls.gcancel.x, controls.gcancel.y) < controls.gcancel.r + 10;
    }
  }, { passive: true });
