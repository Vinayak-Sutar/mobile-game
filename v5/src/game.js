 ...
 *
 * The OS's own answer is used where there is one. Coming back from the
 * background, `innerWidth`/`innerHeight` can still be the shape the window had
 * BEFORE - and a prompt raised on those stale numbers used to stay up with
 * nothing to clear it, so the game had to be loaded afresh from the link (the
 * owner, 2026-09-23). `screen.orientation.type` is never stale, and where the
 * landscape lock was granted it already says landscape, which is the truth
 * about the display whichever way the phone is held.
 */
function isPortraitTouch() {
  ...
  if (!type) return tall;
  // Where the two disagree one of them is stale (or the app is sharing the
  // screen): whichever says landscape wins, so a stale reading can never be
  // what leaves the prompt standing.
...
function checkOrientation() {
  if (!document.body.contains(rotateEl)) return;
...
/**
 * The size and the orientation can both arrive late when the app is brought
 * back, so they are taken again over the next second rather than once.
 */
function recheckOrientation() {
  for (const ms of [0, 150, 400, 900]) setTimeout(resize, ms);
}
...
window.addEventListener('focus', recheckOrientation);
// The OS's own orientation event, where the browser has one.
if (screen.orientation && screen.orientation.addEventListener) {
  screen.orientation.addEventListener('change', recheckOrientation);
}
// And a slow tick while the prompt is up: whatever a browser does or does not
// report, the prompt can never be what is left holding the game.
setInterval(() => { if (rotateEl.classList.contains('on')) checkOrientation(); }, 500);
